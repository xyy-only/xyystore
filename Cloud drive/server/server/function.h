...
int tcpInit(int*);//创建TCP通信socket


int recvFile(int sockfd);//接受，发送文件
int sendFile(int sockfd, const char*);


int recvCycle(int, void*, size_t);//循环接收，发送数据
int sendCycle(int, void*, size_t);


string GenerateStr(int STR_LEN);//产生随机字符串(此处用于生产Salt)


void* ask(void* arg);//轮询函数
