...
unordered_map<string, int>name_to_fd;//用户名，描述符
int epfd = epoll_create(1);//设置epoll

/*轮询函数*/
void* ask(void* arg)
{
	unordered_map<int, time_t>* timeMap = (unordered_map<int, time_t>*)arg;//用户fd上一次发送包实践
	while (1)
	{
		for (auto& e : *timeMap)
		{
			...
			accept_client:
				...
				if (order == "logIn")
				{
					...
					if (s == cipher)
					{
						...
						ret = epoll_ctl(epfd, EPOLL_CTL_ADD, newClient, &ev);
						ERROR_CHECK(ret, -1, "epoll_ctl");
...
else if (order == "puts")
{
				...
				Task task(clients[i], orders, username, Dir);
