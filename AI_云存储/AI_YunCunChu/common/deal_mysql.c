...
MYSQL* msql_conn(char *user_name, char* passwd, char *db_name)
{
    ...
    // 从配置文件读取MySQL主机地址
    char mysql_host[128] = {0};
    if (get_cfg_value(CFG_PATH, "mysql", "ip", mysql_host) != 0)
    {
        // 如果配置中没有ip字段，默认使用localhost
	...
	if ( mysql_real_connect(conn, mysql_host, user_name, passwd, db_name, 0, NULL, 0) == NULL)
	{
