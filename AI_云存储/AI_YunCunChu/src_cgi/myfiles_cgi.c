...
int get_user_files_count(char *user, long *count)
{
    ...
    if (ret == 1)
    {
        // 没有记录集，说明该用户还没有上传过文件，count为0
        LOG(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "user '%s' has no record in user_file_count, count=0\n", user);
        line = 0;
        ret = 0;
        goto END;
    }
...
void handle_user_files_count(char *user)
{
    long line = 0;
    int ret = 0;
    ...
    if (ret == 0)
    {
        return_myfiles_status(line, HTTP_RESP_OK);
    }
    else
    {
        return_myfiles_status(0, HTTP_RESP_FAIL);
    }
}
    ...
    while (FCGI_Accept() >= 0)
    {
        ...
        else
        {
            ...
            if (strcmp(cmd, "count") == 0) // count 获取用户文件个数
            {
                ...
                if (ret == 0)
                {
                    handle_user_files_count(user); //获取用户文件个数
                }
                else
                {
