...
int share_file(char *user, char *md5, char *filename)
{
    ...
    */
    ...
    //查询共享文件数量
    sprintf(sql_cmd, "select count from user_file_count where user = '%s'", FILE_PUBLIC_COUNT);
    int count = 0;
    //返回值： 0成功并保存记录集，1没有记录集，2有记录集但是没有保存，-1失败
    ...
    {
        //插入记录
        sprintf(sql_cmd, "insert into user_file_count (user, count) values('%s', %d)", FILE_PUBLIC_COUNT, 1);
    }
    else if(ret2 == 0)
    {
        //更新用户文件数量count字段
        count = atoi(tmp);
        sprintf(sql_cmd, "update user_file_count set count = %d where user = '%s'", count+1, FILE_PUBLIC_COUNT);
    }
...
END:
    ...
    */
    ...
    {
        out = return_status(HTTP_RESP_DEALFILE_EXIST);
    }
...
int del_file(char *user, char *md5, char *filename)
{
    ...
    */
    ...
    LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC,"%s %d\n", __FUNCTION__, __LINE__);
    //设置数据库编码，主要处理中文编码问题
    mysql_query(conn, "set names utf8");
    ...
    ret2 = rop_zset_exit(redis_conn, FILE_PUBLIC_ZSET, fileid);
    LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC,"%s %d ret2:%d\n", __FUNCTION__, __LINE__, ret2);
    if(ret2 == 1) //存在
    {
        share = 1;  //共享标志
        flag = 1;   //redis有记录
    ...
    {//===2、如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
        ...
        if(ret2 == 0)
        {
            ...
            LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "process_result_one share  = %d\n", share);
        }
        else if(ret2 == -1)//失败
        {
    ...
    ret2 = process_result_one(conn, sql_cmd, tmp); //执行sql语句
    LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC,"%s %d ret2:%d, tmp:%s\n", __FUNCTION__, __LINE__, ret2, tmp);
    if(ret2 != 0)
    {
        LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 操作失败\n", sql_cmd);
        ret = -1;
...
/*
    ...
    LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC,"%s %d, sql_cmd:%s\n", __FUNCTION__, __LINE__, sql_cmd);
    if (mysql_query(conn, sql_cmd) != 0)
    {
        LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 操作失败: %s\n", sql_cmd, mysql_error(conn));
        ret = -1;
...
*/
	...
	{
		count -= 1;
		sprintf(sql_cmd, "update user_file_count set count = %d where user = '%s'", count, user);
 		LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC,"%s %d, sql_cmd:%s\n", __FUNCTION__, __LINE__, sql_cmd);
    	if (mysql_query(conn, sql_cmd) != 0)
    	{
        	LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 操作失败: %s\n", sql_cmd, mysql_error(conn));
...
int pv_file(char *user, char *md5, char *filename)
{
    ...
    int pv = 0;
    int shared_status = 0;
    sprintf(sql_cmd, "select pv, shared_status from user_file_list where user = '%s' and md5 = '%s' and file_name = '%s'", user, md5, filename);

    {
        ...
        if (res_set == NULL)
        {
            LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "mysql_store_result error\n");
            ret = -1;
            goto END;
        }
        ...
        {
            pv = atoi(row[0]);
            shared_status = atoi(row[1]);
        }
        else
        {
            LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 没有记录\n", sql_cmd);
    ...
    //如果文件已分享，同步更新share_file_list表的pv和Redis的FILE_PUBLIC_ZSET score
    if (shared_status == 1)
    {
        sprintf(sql_cmd, "update share_file_list set pv = %d where user = '%s' and md5 = '%s' and file_name = '%s'", pv+1, user, md5, filename);
        if (mysql_query(conn, sql_cmd) != 0)
        {
            LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "%s 操作失败: %s\n", sql_cmd, mysql_error(conn));
            ret = -1;
        ...
        //同步更新Redis中FILE_PUBLIC_ZSET的score
        redis_conn = rop_connectdb_nopwd(redis_ip, redis_port);
        if (redis_conn != NULL)
        {
            char fileid[1024] = {0};
            sprintf(fileid, "%s%s", md5, filename);
            rop_zset_increment(redis_conn, FILE_PUBLIC_ZSET, fileid);
        }
    }
