...
int get_share_files_count(long *pcout)
{
    ...
    if (ret == 1) // 没有记录，说明还没有共享文件，count=0
    {
        line = 0;
        ret = 0;
        goto END;
...
//获取共享文件个数
void handle_get_share_files_count()
{
    long line = 0;
    int ret = 0;
...
int get_ranking_filelist(int start, int count)
{
    ...
    */
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        // 从fileid(md5+filename)中提取md5（fileid前32位是md5）
        char md5[33] = {0};
        strncpy(md5, value[i], 32);
        md5[32] = '\0';

        // 从mysql查询完整的文件信息（user, url, size, type）
        char sql_cmd2[SQL_MAX_LEN] = {0};
        sprintf(sql_cmd2, "select share_file_list.user, file_info.url, file_info.size, file_info.type, share_file_list.create_time from file_info, share_file_list where file_info.md5 = share_file_list.md5 and share_file_list.md5 = '%s' and share_file_list.file_name = '%s' limit 1", md5, filename);

        MYSQL_RES *res_set2 = NULL;
        if (mysql_query(conn, sql_cmd2) == 0)
        {
            res_set2 = mysql_store_result(conn);
            if (res_set2 != NULL)
            {
                MYSQL_ROW row2 = mysql_fetch_row(res_set2);
                if (row2 != NULL)
                {
                    if (row2[0] != NULL)
                        cJSON_AddStringToObject(item, "user", row2[0]);
                    if (row2[1] != NULL)
                        cJSON_AddStringToObject(item, "url", row2[1]);
                    if (row2[2] != NULL)
                        cJSON_AddNumberToObject(item, "size", atol(row2[2]));
                    if (row2[3] != NULL)
                        cJSON_AddStringToObject(item, "type", row2[3]);
                    if (row2[4] != NULL)
                        cJSON_AddStringToObject(item, "create_time", row2[4]);
                }
        ...
        cJSON_AddStringToObject(item, "file_name", filename);
        cJSON_AddNumberToObject(item, "share_status", 1);
...
int main()
{
    ...
    while (FCGI_Accept() >= 0)
    {
        ...
        {
            handle_get_share_files_count(); //获取共享文件个数
        }
        else
        {
